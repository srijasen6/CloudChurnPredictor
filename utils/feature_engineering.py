    ...
    def engineer_features(self, df):
        ...
        try:
            ...
            if 'monthly_charges' in result.columns and 'tenure' in result.columns:
                result['customer_lifetime_value'] = result['monthly_charges'] * result['tenure']
            ...
            if 'total_charges' in result.columns and 'tenure' in result.columns:
                ...
                result['average_monthly_spending'] = result['total_charges'] / result['tenure'].replace(0, 1)
            ...
            if 'number_of_dependents' in result.columns and 'monthly_charges' in result.columns:
                result['charges_per_dependent'] = result['monthly_charges'] / (result['number_of_dependents'] + 1)
            ...
            if 'tenure' in result.columns and 'monthly_charges' in result.columns:
                result['tenure_charges_interaction'] = result['tenure'] * result['monthly_charges']
...
class RatioTransformer(BaseEstimator, TransformerMixin):
    ...
    def transform(self, X):
        ...
        """
        X_ = X.copy()
        X_[self.name] = X_[self.numerator] / (X_[self.denominator] + self.offset)
        return X_
